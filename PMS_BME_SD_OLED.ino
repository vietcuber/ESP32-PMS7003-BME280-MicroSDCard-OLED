...
void displayDataToSerial(){
  ...
  Serial.print("Temperature: ");    Serial.println(last_Temperature);
  Serial.print("Pressure: ");    Serial.println(last_Pressure);
  Serial.print("Altitude: ");    Serial.println(last_Altitude);
  Serial.print("Humidity: ");    Serial.println(last_Humidity);
  Serial.println("----------------------------");
}

void drawFullInfoScreen() {
  ...
  display.print("PM1.0: "); display.print(last_PM1);
  
  display.setCursor(0, 10);
  display.print("PM2.5: "); display.print(last_PM2_5);
  
  display.setCursor(0, 20);
  display.print("PM10 : "); display.print(last_PM10);

  // --- CỘT PHẢI (ENVIRONMENT) - Căn lề tại x = 64 ---
  display.setCursor(64, 0);
  display.print("T:"); display.print(last_Temperature, 1); display.print("C"); // Temp
  
  display.setCursor(64, 10);
  display.print("H:"); display.print(last_Humidity, 0); display.print("%");  // Humidity
  
  display.setCursor(64, 20);
  display.print("P:"); display.print(last_Pressure/100, 0); display.print("h"); // Pressure (hPa)

  display.setCursor(64, 30);
  display.print("A:"); display.print(last_Altitude, 0); display.print("m");  // Altitude

  // Vẽ đường kẻ ngang phân cách
  display.drawLine(0, 45, 128, 45, SSD1306_WHITE);

  // 1. Logic hiển thị "Saved"
  if (showSaved && millis() < savedUntil) {
    display.setCursor(0, 50);
    display.print("SAVED!"); 
  } else {
    // Nếu hết thời gian saved, tắt cờ
    if (showSaved) showSaved = false; 
    
    // Hiển thị Spinner và Thời gian chạy khi không "Saved"
    // Spinner
    display.setCursor(0, 50);
...
{
  Serial.begin(115200);
  Serial.println("ESP32 khởi động. PMS7003 Khởi động. BME280 khởi động. OLED khởi động. SDCard khởi động");
  ...
  if (!SD.begin(SDCARD_CS)) {
    ...
    if (myFile) {
      ...
      if (myFile.size() == 0) {
        myFile.println("Time(s), PM1.0, PM2.5, PM10, Temperature, Pressure, Altitude, Humidity");
...
void loop(){
  ...
  if(currentTime - previousDataMillis >= dataInterval){
    ...
    last_Temperature = bme.readTemperature();
    last_Pressure = bme.readPressure() / 100.0F;
    last_Altitude = bme.readAltitude(1013.25);
    last_Humidity = bme.readHumidity();

    displayDataToSerial();

    String dataString = String(millis()/1000) + "," + String(last_PM1) + "," + String(last_PM2_5) + "," + String(last_PM10) + "," + String(last_Temperature) + "," + String(last_Pressure) + "," + String(last_Altitude) + "," + String(last_Humidity);
